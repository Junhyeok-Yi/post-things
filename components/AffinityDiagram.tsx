...
export default function AffinityDiagram({
  ...
  // 주제별 그룹화 함수 (스마트 토픽 분류 적용) - 향후 사용 예정
    ...
    switch (sortType) {
      case 'category':
        ...
        const sortedCategories = Object.keys(groupedByCategory).sort((a, b) => {
          return getCategoryPriority(a as Category) - getCategoryPriority(b as Category);
        });
                  ...
                  {groupNotes.map((note) => (
                    ...
                    >
                      ...
                      <div className={`relative w-full h-full rounded-2xl shadow-lg transition-all duration-300 ${
                        ...
                        {/* 완료 오버레이 (M2Z1 스타일) - Hover 시 투명해짐 */}
                        {note.isCompleted && (
                          <div className="absolute inset-0 bg-black/60 rounded-2xl flex items-center justify-center z-10 backdrop-blur-sm transition-all duration-300 group-hover:bg-black/20 group-hover:backdrop-blur-none">
                            <div className="text-center transition-opacity duration-300 group-hover:opacity-50">
